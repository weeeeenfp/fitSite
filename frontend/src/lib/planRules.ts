...
function fullBodyDay(day: WeekDay, intensity: Intensity, variant: "A" | "B" | "C" = "A"): PlanDay {
  const templates: Record<"A" | "B" | "C", { focus: string; items: PlanItem[] }> = {
    A: {
      focus: "全身循環 A｜深蹲＋推＋拉",
      items: [
        weightItem("深蹲", 3, "8-10", intensity, "重量抓能標準完成組數的重量"),
        weightItem("臥推", 3, "8-10", intensity, "掌握好肩胛穩定再加重"),
        weightItem("槓鈴划船", 3, "8-10", intensity, "背部夾緊，避免用甩的"),
        weightItem("核心捲腹", 3, "12-15", intensity, "動作放慢，避免用脖子代償"),
      ],
    },
    B: {
      focus: "全身循環 B｜硬舉＋肩推＋滑輪",
      items: [
        weightItem("硬舉", 3, "6-8", intensity, "背打直，重量寧可保守"),
        weightItem("肩推", 3, "8-10", intensity, "核心收緊避免腰椎代償"),
        weightItem("滑輪下拉", 3, "10-12", intensity, "肩胛下沉再拉"),
        weightItem("核心捲腹", 3, "12-15", intensity, "動作放慢"),
      ],
    },
    C: {
      focus: "全身循環 C｜腿推＋上斜臥推＋引體",
      items: [
        weightItem("腿推", 3, "10-12", intensity, "膝蓋方向對齊腳尖"),
        weightItem("上斜臥推", 3, "8-10", intensity, "訓練上胸"),
...
function pushDay(day: WeekDay, intensity: Intensity): PlanDay {
  return {
    ...
    focus: "推｜胸肩三頭",
    items: [
      weightItem("臥推", 4, "6-10", intensity, "主項目，力竭前1-2下停止"),
      weightItem("上斜臥推", 3, "8-12", intensity, "加強上胸"),
      weightItem("肩推", 3, "8-10", intensity, "核心收緊"),
...
function pullDay(day: WeekDay, intensity: Intensity): PlanDay {
  return {
    ...
    items: [
      ...
      weightItem("滑輪下拉", 3, "10-12", intensity, "肩胛下沉再拉"),
      weightItem("二頭彎舉", 3, "10-15", intensity, "避免甩動借力"),
    ],
  };
}
...
function legDay(day: WeekDay, intensity: Intensity): PlanDay {
  return {
    ...
    focus: "腿｜下肢",
    items: [
      weightItem("深蹲", 4, "6-10", intensity, "主項目，力竭前1-2下停止"),
...
function upperDay(day: WeekDay, intensity: Intensity): PlanDay {
  return {
    ...
    focus: "上肢｜胸背肩手臂",
    items: [
      weightItem("臥推", 3, "8-10", intensity, "主項目"),
      weightItem("槓鈴划船", 3, "8-10", intensity, "背部夾緊"),
      weightItem("肩推", 3, "8-10", intensity, "核心收緊"),
      weightItem("二頭彎舉", 2, "10-15", intensity, "收尾"),
