...
import { CARDIO_MET, workoutKcal } from "../../lib/calorie";
...
const NAMES = Object.keys(CARDIO_MET);

export default function CardioForm({ date, bodyWeightKg, initial, onSave, onCancel }: CardioFormProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const kcal = workoutKcal("cardio", name, intensity, minutes, bodyWeightKg);
    onSave({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      <p className="text-sm text-neutral-500">
        預估消耗：<span className="font-semibold text-neutral-800 dark:text-neutral-100">{workoutKcal("cardio", name, intensity, minutes, bodyWeightKg)} kcal</span>
      </p>
